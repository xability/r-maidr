...
- Step plots — `geom_step()` in ggplot2, `plot(type = "s")` / `plot(type = "S")`
  ...
  hypnogram
- Box plots
- Violin plots (ggplot2; Base R `vioplot::vioplot()` [experimental])
- Candlestick (OHLC) charts — ggplot2 via {tidyquant} (with optional
  ...
  native graphics)
- Heatmaps
- Contour plots (Base R `contour()`; ggplot2 `geom_contour()` [experimental])
...
- ggplot2: area and stacked area charts [experimental], contour plots
  [experimental], error bars [experimental], Gantt charts via `maidr_gantt()`
  [experimental], hexbin plots [experimental] and ROC curves via
  `maidr_roc()` [experimental]
...
[README](https://r.maidr.ai/#supported-plot-types).
