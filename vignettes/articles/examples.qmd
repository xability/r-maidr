...
> **Attach order matters.** `library(quantmod)` after `library(maidr)`
> puts `package:quantmod` ahead of `package:maidr` on the search path,
> so a bare `chartSeries()` call reaches quantmod directly and maidr
> never records it — the chart draws, but `show()` and `save_html()`
> then report that no Base R plot was found. Either attach `quantmod`
> **before** `maidr`, or call `maidr::chartSeries()` explicitly as
> below, which works in either order.
