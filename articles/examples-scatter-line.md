...
chart, and sonification plays the y value as pitch as you move along x.
A regression plot layers a fitted smooth over the points, so the fit is
its own navigable layer. Scatter, line, step and smooth layers are all
stable plot types (see “Supported plot types” in the README). The
[examples hub](https://r.maidr.ai/articles/examples.md) lists every
other plot family.
