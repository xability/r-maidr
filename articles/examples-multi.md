...
These are the composite plots: several layers on one set of axes,
several independent panels in one figure, or one chart repeated per
subgroup. maidr keeps each layer and each panel separate, so a reader
switches layer or panel with the keyboard and hears each one sonified on
its own axes rather than as one merged series. Faceted plots,
multi-panel layouts and multi-layered plots are stable plot types (see
“Supported plot types” in the README). The [examples
