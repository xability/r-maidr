...
equivalent, so maidr reads each by mapping it onto a layer type it
already knows: a correlogram becomes a lollipop walked lag by lag, a
periodogram or seasonal subseries a line, a lag plot or Q-Q plot a point
layer, and the Left and Right arrow keys and sonification then work
exactly as they do for those types. Every reading on this page is an
experimental plot type (see “Experimental Plot Types” in the README):
none has been through a user study and each may change without a
