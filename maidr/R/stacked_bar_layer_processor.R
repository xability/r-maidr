...
StackedBarLayerProcessor <- R6::R6Class("StackedBarLayerProcessor",
  ...
  public = list(
    ...
    needs_reordering = function() {
      return(TRUE)  # Stacked bars need reordering
    },
    
    #' Apply reordering to plot
    apply_reordering = function(plot) {
      # Check if this is a stacked bar plot
      if (!self$is_stacked_bar_plot(plot)) {
      ...
      # Apply reordering
      reordered_data <- self$reorder_for_stacked_bar(plot$data, fill_col, category_col)
