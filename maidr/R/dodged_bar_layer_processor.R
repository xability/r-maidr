...
DodgedBarLayerProcessor <- R6::R6Class("DodgedBarLayerProcessor",
  ...
  public = list(
    ...
    needs_reordering = function() {
      TRUE # Dodged bars need reordering
    },
    ...
    #' Reorder only this layer's data: per x, reverse fill order for right-first DOM
    reorder_layer_data = function(data, plot) {
      ...
      if (is.null(x_col) || is.null(fill_col) ||
          !(x_col %in% names(data)) || !(fill_col %in% names(data))) return(data)
      # Ensure deterministic category order
      x_values <- sort(unique(data[[x_col]]))
      fill_values <- sort(unique(data[[fill_col]]))
      out <- data.frame()[0, names(data), drop = FALSE]
      for (xv in x_values) {
        for (fv in rev(fill_values)) {
          rows <- data[data[[x_col]] == xv & data[[fill_col]] == fv, , drop = FALSE]
          if (nrow(rows) > 0) out <- rbind(out, rows)
        }
      }
      out
    },
    ...
    extract_data_impl = function(plot, built = NULL) {
      ...
      if (is.null(built)) built <- ggplot_build(plot)
