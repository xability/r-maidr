  ...
  public = list(
    ...
    extract_panels = function() {
      ...
      for (i in seq_len(nrow(panel_layout))) {
        ...
        # The gtable panel names may include suffixes (e.g., panel-1-1.10-7-10-7), so we match by prefix
        expected_panel_name <- paste0("panel-", panel_info$ROW, "-", panel_info$COL)
        gtable_panel_name <- NULL
        prefix_matches <- which(grepl(paste0("^", expected_panel_name, "\\b"), self$gt$layout$name))
        if (length(prefix_matches) > 0) {
          gtable_panel_name <- self$gt$layout$name[prefix_matches[1]]
        }
        ...
        panels[[i]] <- list(
          ...
          gtable_panel_name = gtable_panel_name
        )
      }
    ...
    process_panel = function(panel) {
      ...
      for (layer_idx in seq_along(self$plot$layers)) {
        ...
        if (!is.null(processor)) {
          ...
          panel_name <- if (!is.null(panel$gtable_panel_name)) panel$gtable_panel_name else paste0("panel-", panel$row, "-", panel$col)
          # Confirm it resolves to a panel row in gt (no logging)
          panel_ctx <- list(
    ...
    # Removed old grob-id lookup helpers; panel-scoped selection is handled in processors via panel_ctx
