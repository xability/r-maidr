...
this function relies on. Falls back to "Series \<id\>" when the mapped
column is not present on the plot data (for example an expression such
as `aes(colour = paste(a, b))`).
