...
– and the line processor this class inherits already decides those, for
its own reason: it keeps the rows ggplot2 draws through, dropping
leading and trailing NA-y rows and announcing an interior one as missing
(`line_drawn_span()`). Repeating that here would be a second filter with
a second rationale over the same rows. Raised in review on \#169; the
