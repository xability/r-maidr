...
# Create the multi-layer plot with different y-axis for each layer
p_dual_axis <- ggplot(dual_plot_data, aes(x = x)) +
  # Bar chart
  geom_bar(aes(y = bar_values), stat = "identity", fill = "skyblue", alpha = 0.7) +
  # Line chart with its own y mapping
  geom_line(aes(y = line_values), color = "red", linewidth = 1) +
  # Labels and title
  labs(
    title = "Multi-Layer Plot Example",
    x = "X values"
  ) +
  theme_minimal()
