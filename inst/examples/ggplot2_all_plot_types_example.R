...
library(ggplot2)
# load_all() lives in pkgload; devtools only re-exports it. Depending on
# pkgload directly lets this script run in an environment that has the
# package-loading tooling without the whole devtools stack.
library(pkgload)
library(patchwork)
...
if (have_tidyquant) {
  ...
  cat("      Install with: install.packages(\"tidyquant\")\n")
}
...
if (have_tidyquant) {
  ...
  cat("\n=== TESTS 19-20 SKIPPED: tidyquant not installed ===\n")
  cat("Candlestick baselines left as they are; every other baseline was regenerated.\n")
}
...
if (have_tidyquant) {
  ...
  cat("- Candlestick plot (OHLC): SKIPPED (tidyquant not installed)\n")
  cat("- Candlestick + MA + volume: SKIPPED (tidyquant not installed)\n")
}
